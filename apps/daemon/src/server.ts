...
//   - the ACP forced-shutdown override is scoped to known post-completion
//     shutdown signals + clean completion only (signed-32-bit-overflow
//     SIGKILL or non-clean ACP state still report `failed`).
...
export function classifyChatRunCloseStatus(params: {
  ...
  const acpForcedShutdown =
    ...
    (
      ...
      // teardown as a normal exit code 130 instead of a SIGTERM/SIGABRT signal.
      // Once the ACP prompt already resolved cleanly, that is shutdown noise
      // rather than an agent execution failure.
